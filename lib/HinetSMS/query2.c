...
*	Description : notify2 <Ip Addr> <Login> <Pw> <MessageID>
*  compiler for Unix  : gcc -o query2 -lsocket query2.c
*  compiler for Linux : gcc -o query2 query2.c
**********************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
if(argc<5)
{
   printf("\nUsage : query2 <Ip Addr> <Login> <Pw> <MsgID> ");
   printf("\n <ex>: query2 202.39.54.130 test test1 A8485839053681622407");
   printf("\nOutput: query2.log");
...
/******* 查詢傳送結果 *******/
memset((char *)&sendMsg, 0, sizeof(sendMsg));
sendMsg.msg_type = 2;   //訊息型態 2 -> 查詢傳送結果
